...
// utils
    ...
    thaiMoment.fn.thaiYear = function (input) {
        ...
        if (typeof input === 'number') {
            tw = toThai(this.year(), this.month(), this.date());
            lastDay = Math.min(tw.year, thaiMoment.twDaysInMonth(input, tw.month));
    ...
    /************************************
     twMoment Statics
     ************************************/
    thaiMoment.twDaysInMonth = function (year, month) {
        year += div(month, 12);
        month = mod(month, 12);
        if (month < 0) {
            month += 12;
    ...
    thaiMoment.twIsLeapYear = thai.isLeapYear;

    thaiMoment.twConvert = {
        toThai: toThai,
